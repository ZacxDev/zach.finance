        ...
        <div>
          ...
          <div class="flex">
            <div class="flex flex-col gap-4">
              <div>
                <div v-if="portfolio !== null" class="flex flex-col items-start">
...
export default defineComponent({
  setup() {
    ...
    const defaultStartDateTimeString = session.value ? new Date(session.value.startDate * 1000).toISOString().substring(0, 10) : undefined
    const defaultEndDateTimeString = session.value ? new Date(session.value.endDate * 1000).toISOString().substring(0, 10) : undefined

    onResult((s) => {
      const ss = s.data.getSession
      session.value = ss

      startDateString.value = new Date(ss.startDate * 1000).toISOString().substring(0, 10)
      endDateString.value = new Date(ss.endDate * 1000).toISOString().substring(0, 10)
    })
